...
def mask_target_part(pos_proposals_list, pos_assigned_gt_inds_list, num_parsing, gt_masks_list,
                cfg):
    ...
    mask_targets = [map(mask_target_single, pos_proposals_list,
                       pos_assigned_gt_inds_list, [gt_masks_list[0][i]], cfg_list) for i in range(num_parsing)]
    mask_targetsNew = []
    for i in range(num_parsing):
        mask_target = list(mask_targets[i])
        if len(mask_target) > 0:
            mask_target = torch.cat(mask_target)
        mask_targetsNew.append(mask_target)
    return mask_targetsNew
