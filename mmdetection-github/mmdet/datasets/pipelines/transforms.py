...
class ResizePart:
    ...
    def _resize_masks(self, results):
        ...
        for key in results.get('mask_fields', []):
            for i in range(0, 3):
                if results[key][i] is None:
                    continue
                if self.keep_ratio:
                    results[key][i] = results[key][i].rescale(results['scale'])
                else:
                    results[key][i] = results[key][i].resize(results['img_shape'][:2])
...
class RandomFlipPart:
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('mask_fields', []):
                for i in range(0,3):
                    results[key][i] = results[key][i].flip(results['flip_direction'])
...
class CopyPasteParsing:
    ...
    def __call__(self, results):
        ...
        if self.selected:
            ...
            selected_results = results['mix_results'][0]
        return self._copy_paste(results, selected_results, self.ratio)
    ...
    def _select_object(self, results):
        ...
        bitmasks = []
        for mask in masks:
            bitmap = mask.to_ndarray().astype(int)
            bitmask = BitmapMasks(bitmap, mask.height, mask.width)
            bitmasks.append(bitmask)
        
        max_num_pasted = min(bboxes.shape[0] + 1, self.max_num_pasted)
        num_pasted = np.random.randint(2, max_num_pasted)
        selected_inds = np.random.choice(
            bboxes.shape[0], size=num_pasted, replace=False)
        ...
        for bitmask in bitmasks:
            selected_mask = bitmask[selected_inds]
    ...
    def _copy_paste(self, dst_results, src_results, ratio):
        """CopyPaste transform function.
        ...
        dst_bitmasks = []
        for mask in dst_masks:
            bitmap = mask.to_ndarray().astype(int)
            bitmask = BitmapMasks(bitmap, mask.height, mask.width)
            dst_bitmasks.append(bitmask)
        ...
        for src_mask in src_masks:
            src_mask2.append(src_mask.masks)
        src_mask2 = np.concatenate(src_mask2)
        
        # update masks and generate bboxes from updated masks
        composed_mask = np.where(np.any(src_mask2, axis=0), 1, 0)
        
        updated_dst_masks = []
        for dst_bitmask in dst_bitmasks:
            updated_dst_mask = self.get_updated_masks(dst_bitmask, composed_mask)
            updated_dst_masks.append(updated_dst_mask)
        
        #pdb.set_trace()
        instance_masks = []
        for i in range(updated_dst_masks[0].masks.shape[0]):
            instance_mask= np.zeros(updated_dst_masks[0].masks.shape[1:])
            for j in range(len(updated_dst_masks)):
                instance_mask = np.logical_or(instance_mask, updated_dst_masks[j].masks[i])
            instance_masks.append(instance_mask)
        #pdb.set_trace()
        instance_dst_masks = BitmapMasks(np.stack(instance_masks).astype(int), instance_mask.shape[0], instance_mask.shape[1])
        
        updated_dst_bboxes = instance_dst_masks.get_bboxes()
        assert len(updated_dst_bboxes) == len(updated_dst_masks[0].masks)
        ...
        for i in range(updated_dst_masks[0].masks.shape[0]):
            for j in range(len(updated_dst_masks)):
                if np.count_nonzero(updated_dst_masks[j].masks[i]==1) < self.mask_occluded_thr:
                    updated_dst_masks[j].masks[i][:]=0
            if np.count_nonzero(updated_dst_masks[2].masks[i]==1) ==0 and np.count_nonzero(updated_dst_masks[3].masks[i]==1) == 0:
        ...
        bboxes = np.concatenate([updated_dst_bboxes[valid_inds], src_bboxes])
        labels = np.concatenate([dst_labels[valid_inds], src_labels])
        updated_dst_masks = [updated_dst_mask.masks[valid_inds] for updated_dst_mask in updated_dst_masks]
        
        #new_image = cv2.cvtColor(np.uint8(img), cv2.COLOR_BGR2RGB)
        #cv2.imwrite("test.jpg", img)
        
        gt_masks = []
        #new_shapes = []
        for i in range(len(updated_dst_masks[0])):
            gt_mask = []
            for j in range(len(updated_dst_masks)):
                #mask = bitmap_to_polygon(updated_dst_masks[j][i])
                mask = updated_dst_masks[j][i]
                ...
                for contour in contours:
                    ...
                    # Convert the polygon to a numpy array and append it to the list
                    polygons.append(approx.squeeze())
                #pdb.set_trace()
                polygon_coco = []
                for k in range(len(polygons)):
                    if len(polygons[k])>3:
                        polygon_coco.append(np.asarray(polygons[k]).flatten().tolist())
                        #new_shape = dict()
                        #new_shape.update({'label': j})
                        #new_shape.update({'points': polygons[k].tolist()})
                        #new_shape.update({'group_id': i})
                        #new_shape.update({'shape_type': 'polygon'})
                        #new_shape.update({'flags': {}})
                        #new_shapes.append(new_shape)
                gt_mask.append(polygon_coco)
            gt_masks.append(gt_mask)
            
        #pdb.set_trace()
        
        for i in range(src_masks[0].masks.shape[0]):
            gt_mask = []
            for j in range(len(src_masks)):
                #mask = bitmap_to_polygon(src_masks[j].masks[i])
                mask = src_masks[j].masks[i]
                contours, _ = cv2.findContours(np.uint8(mask), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                polygons = []
                # Iterate over each contour
                for contour in contours:
        ...
        gt_masks = [PolygonMasks(
                [self.process_polygons(polygons[i]) for polygons in gt_masks], dst_masks[0].height,
                dst_masks[0].width) for i in range(0, 5)]

        dst_results['img'] = img
        dst_results['gt_bboxes'] = bboxes
        dst_results['gt_labels'] = labels
        dst_results['gt_masks'] = gt_masks
