...
class FeatureBlendHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                ...
                pretrained=None,
                init_cfg=None):
        super(FeatureBlendHead, self).__init__(
    ...
    def forward_train(self,
        ...
        if self.fusion_head is not None:
            #pdb.set_trace()
            fusion_results = self._fusion_forward_train(seg_results['seg_pred'], mask_results['parsing_pred'], sampling_results, gt_masks, img_metas)
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks,
        ...
        mask_results.update(loss_mask=loss_mask, mask_targets=mask_targets, parsing_pred=parsing_pred)
        return mask_results
    ...
    def _seg_forward_train(self, x, gt_masks, img_metas):
        ...
        seg_targets = self.semantic_head.get_targets(gt_masks, device, self.train_cfg, pad_shape)
        loss_seg = self.semantic_head.loss(seg_results['seg_pred'], seg_targets)

        seg_results.update(loss_seg=loss_seg, seg_targets=seg_targets)
        return seg_results
        
    def _seg_forward(self, x, is_train):
        #pdb.set_trace()
        seg_pred, seg_features = self.semantic_head(x, is_train)
        seg_results = dict(seg_pred=seg_pred, seg_features=seg_features)
        return seg_results
        
    def _fusion_forward_train(self, seg_pred, mask_pred, sampling_results, gt_masks, img_metas):
        #pdb.set_trace()
        pad_shape = img_metas[0]['pad_shape']
        fusion_results = self._fusion_forward(seg_pred, mask_pred, pad_shape, [res.pos_bboxes for res in sampling_results][0], True)
        ...
        #fusion_results['masks_nan'] = fusion_results['masks_nan'].to(device)
        #pdb.set_trace()
        loss_fusion = self.fusion_head.loss(fusion_results['final_pred'], fusion_targets)
        
        fusion_results.update(loss_fusion=loss_fusion, fusion_targets = fusion_targets)
        return fusion_results
        
    def _fusion_forward(self, seg_pred, mask_pred, pad_shape, boxes, is_Train):
        #pdb.set_trace()
        device = seg_pred.device
        seg_shape = seg_pred.shape
        if is_Train:
            seg_pred = seg_pred.detach()
            #seg_pred.requires_grad=True
        ...
        for i in range(len(boxes)):
            boxes[i] = boxes[i]/4
            #temp_pred = seg_pred[0].clone()
            temp_pred = torch.zeros(seg_shape[1], seg_shape[2], seg_shape[3], device=device, dtype=torch.float32)
            #temp_pred = torch.full((seg_shape[1], seg_shape[2], seg_shape[3]), float('nan'), device=device, dtype=torch.float32)
            #temp_pred[:,(int)(boxes[i][1]): (int)(boxes[i][3]), (int)(boxes[i][0]): (int)(boxes[i][2])] = temp_pred[:,(int)(boxes[i][1]): (int)(boxes[i][3]), (int)(boxes[i][0]): (int)(boxes[i][2])] + seg_pred[0][:,(int)(boxes[i][1]): (int)(boxes[i][3]), (int)(boxes[i][0]): (int)(boxes[i][2])]
            #fusion_pred.append(torch.cat((mask_pred[i],temp_pred),0))
            temp_pred[:,(int)(boxes[i][1]): (int)(boxes[i][3]), (int)(boxes[i][0]): (int)(boxes[i][2])] = mask_pred[i][:,(int)(boxes[i][1]): (int)(boxes[i][3]), (int)(boxes[i][0]): (int)(boxes[i][2])] + seg_pred[0][:,(int)(boxes[i][1]): (int)(boxes[i][3]), (int)(boxes[i][0]): (int)(boxes[i][2])]
            #mask = ~torch.isnan(temp_pred)
            #masks_nan.append(mask)
            fusion_pred.append(temp_pred)
        #pdb.set_trace()
        fusion_pred = torch.stack(fusion_pred,0)
        #masks_nan = torch.stack(masks_nan,0)
        fusion_pred.requires_grad = True
        #mask_pred = mask_pred_former.clone().detach().to(device)  
        #fusion_pred = torch.cat((mask_pred,seg_pred),1)
        final_pred = self.fusion_head(fusion_pred, pad_shape, is_Train)
    ...
    def simple_test(self,
        ...
        if not self.with_mask:
            ...
            if det_bboxes[0].shape[0] == 0:
                ...
                if self.fusion_head is not None:
                    pdb.set_trace()
                    fusion_results = self._fusion_forward(seg_results['seg_pred'], mask_results[0], pad_shape, det_bboxes[0][:, :4], False)
                    #segm_results.append(self.simple_get_targets(fusion_results['final_pred'].to('cuda:0'), self.test_cfg, ori_shape))
                    #final_pred = torch.nan_to_num(fusion_results['final_pred'], nan=-1.0)
                    segm_results.append(self.mask_head.get_seg_masks(
                        fusion_results['final_pred'].to('cuda:0'), det_bboxes[0][:, :4], det_labels[0], self.test_cfg, ori_shape[i], 1.0, True))
                else:
