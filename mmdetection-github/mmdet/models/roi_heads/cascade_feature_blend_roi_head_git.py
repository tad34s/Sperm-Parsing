...
class CascadeRoIHeadGit(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(
        ...
        self.semantic_traced_model = torch.jit.load(
            "cascadercnnfusion_tracedsemantic.pt"
        )
    ...
    def _mask_forward_train(
        self, x, sampling_results, gt_masks, rcnn_train_cfg, img_metas, bbox_feats=None
    ):
        ...
        parsing_pred = self.mask_head.parsing_results(
            mask_results["mask_pred"],
            [res.pos_bboxes for res in sampling_results][0],
            pad_shape,
            4.0,
            True,
        )

        loss_mask, loss_mask2, loss_mask3, loss_mask4 = self.mask_head.loss(
            mask_results["mask_pred"],
            mask_targets,
            mask_results["mask_pred2"],
            mask_edges,
            pos_labels,
        )
        ...
        mask_results.update(
            ...
            loss_mask2=loss_mask2,
            loss_mask3=loss_mask3,
            loss_mask4=loss_mask4,
        )
        # pdb.set_trace()
        return mask_results
    ...
    def _seg_forward_train(self, x, gt_masks, img_metas):
        ...
        seg_targets, gt_edges, seg_instance_targets, gt_instance_edges = (
            self.semantic_head.get_targets(gt_masks, device, self.train_cfg, pad_shape)
        )
        # pdb.set_trace()
        (
            loss_seg,
            ...
            loss_seg8,
            loss_seg9,
        ) = self.semantic_head.loss(
            seg_results["seg_pred"],
            seg_targets,
            seg_results["seg_edges"],
            gt_edges,
            seg_results["seg_pred2"],
            seg_instance_targets,
            seg_results["seg_edges2"],
            gt_instance_edges,
        )
    ...
    def _seg_forward(self, x, is_train):
        ...
        seg_pred, seg_pred2, seg_features, seg_edges, seg_edges2 = (
            self.semantic_traced_model(x[0], x[1], x[2], x[3], x[4])
        )
        seg_results = dict(
            seg_pred=seg_pred,
            seg_pred2=seg_pred2,
            ...
            seg_edges=seg_edges,
            seg_edges2=seg_edges2,
        )
        return seg_results

    def _fusion_forward_train(
        self, seg_pred, mask_pred, seg_features, sampling_results, gt_masks, img_metas
    ):
        # pdb.set_trace()
        pad_shape = img_metas[0]["pad_shape"]
        fusion_results = self._fusion_forward(
            seg_pred,
            mask_pred,
            seg_features,
            pad_shape,
        ...
        fusion_targets, fusion_edges, fusion_instance_targets = (
            self.fusion_head.get_targets(gt_masks, device, sampling_results, pad_shape)
        )
        # pdb.set_trace()
        (
            loss_fusion,
            ...
            loss_fusion6,
            loss_fusion7,
        ) = self.fusion_head.loss(
            fusion_results["final_pred"],
            fusion_targets,
            fusion_results["final_edge"],
            fusion_edges,
            fusion_results["final_pred2"],
            fusion_instance_targets,
        )

        fusion_results.update(
            loss_fusion=loss_fusion,
            fusion_targets=fusion_targets,
            loss_fusion2=loss_fusion2,
            loss_fusion4=loss_fusion4,
            loss_fusion5=loss_fusion5,
            loss_fusion6=loss_fusion6,
            loss_fusion7=loss_fusion7,
        )
        return fusion_results

    def _fusion_forward(
        self, seg_pred, mask_pred, seg_features, pad_shape, boxes, is_Train
    ):
        # pdb.set_trace()
        # if is_Train:
        ...
        for i in range(mask_pred.shape[0]):
            mask_attn = torch.where(
                mask_pred[i].unsqueeze(0) == 0,
                torch.tensor(float(torch.min(mask_pred[i]))).cuda(),
        ...
        for i in range(self.num_stages):
            ...
            if i == self.num_stages - 1:
                ...
                if self.fusion_head is not None:
                    # pdb.set_trace()
                    fusion_results = self._fusion_forward_train(
                        seg_results["seg_pred2"],
                        mask_results["parsing_pred"],
                        seg_results["seg_features"],
                        sampling_results,
                        gt_masks,
                        img_metas,
                    )
            ...
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                for i in range(num_imgs):
                    if det_bboxes[i].shape[0] == 0:
                        ...
                        fusion_results["final_pred"] = F.interpolate(
                            fusion_results["final_pred"],
                            size=pad_shapes[i][:2],
                            mode="bilinear",
                            align_corners=True,
    ...
    def simple_get_targets(self, mask_pred, rcnn_test_cfg, ori_shape):
        ...
        cls_segms = [
            [] for _ in range(self.mask_head.num_parsing)
    ...
    def onnx_export(self, x, proposals, img_metas):
        ...
        for i in range(self.num_stages):
            ...
            cls_score = cls_score.reshape(
                batch_size, num_proposals_per_img, cls_score.size(-1)
            )
        ...
        if not self.with_mask:
            ...
            segm_results = segm_results.reshape(
                batch_size, det_bboxes.shape[1], max_shape[0], max_shape[1]
            )
