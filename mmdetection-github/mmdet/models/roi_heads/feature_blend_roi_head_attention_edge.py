...
class FeatureBlendHeadAttentionEdge(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                ...
                pretrained=None,
                init_cfg=None):
        super(FeatureBlendHeadAttentionEdge, self).__init__(
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks,
        ...
        #parsing_pred = mask_results['mask_features']
    ...
    def _seg_forward_train(self, x, gt_masks, img_metas):
        ...
        seg_targets, gt_edges, seg_instance_targets = self.semantic_head.get_targets(gt_masks, device, self.train_cfg, pad_shape)
        #pdb.set_trace()
        loss_seg, loss_seg2, loss_seg3, loss_seg4, loss_seg5, loss_seg6, loss_seg7 = self.semantic_head.loss(seg_results['seg_pred'], seg_targets, seg_results['seg_edges'], gt_edges, seg_results['seg_pred2'], seg_instance_targets)

        seg_results.update(loss_seg=loss_seg, seg_targets=seg_targets, loss_seg2=loss_seg2, loss_seg3=loss_seg3, loss_seg4=loss_seg4, loss_seg5 = loss_seg5, loss_seg6 = loss_seg6, loss_seg7 = loss_seg7)
        return seg_results
        
    def _seg_forward(self, x, is_train):
        #pdb.set_trace()
        seg_pred, seg_pred2, seg_features, seg_edges = self.semantic_head(x, is_train)
        seg_results = dict(seg_pred=seg_pred, seg_pred2=seg_pred2, seg_features=seg_features, seg_edges=seg_edges)
        return seg_results
    ...
    def _fusion_forward_train(self, seg_pred, mask_pred, seg_features, sampling_results, gt_masks, img_metas):
        ...
        #pdb.set_trace()
        loss_fusion, loss_fusion2, loss_fusion3, loss_fusion4, loss_fusion5, loss_fusion6, loss_fusion7 = self.fusion_head.loss(fusion_results['final_pred'], fusion_targets, fusion_results['final_edge'], fusion_edges, fusion_results['final_pred2'], fusion_instance_targets)
        
        fusion_results.update(loss_fusion=loss_fusion, fusion_targets = fusion_targets, loss_fusion2=loss_fusion2, loss_fusion3 = loss_fusion3, loss_fusion4 = loss_fusion4, loss_fusion5 = loss_fusion5, loss_fusion6 = loss_fusion6, loss_fusion7 = loss_fusion7)
    ...
    def _fusion_forward(self, seg_pred, mask_pred, seg_features, pad_shape, boxes, is_Train):
        ...
        fusion_results = dict(final_pred = final_pred, final_edge = final_edge, final_pred2 = final_pred2)
        #fusion_results = dict()
        return fusion_results
    ...
    def simple_test(self,
        ...
        if not self.with_mask:
            ...
            if det_bboxes[0].shape[0] == 0:
                ...
                if self.fusion_head is not None:
                    #pdb.set_trace()
                    fusion_results = self._fusion_forward(seg_results['seg_pred2'], mask_results[0], seg_results['seg_features'], pad_shape, det_bboxes[0][:, :4], False)
