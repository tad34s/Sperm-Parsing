...
class FeatureBlendHeadAttentionInstance(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                ...
                pretrained=None,
                init_cfg=None):
        super(FeatureBlendHeadAttentionInstance, self).__init__(
    ...
    def _seg_forward_train(self, x, gt_masks, img_metas):
        ...
        seg_targets, seg_instance_targets = self.semantic_head.get_targets(gt_masks, device, self.train_cfg, pad_shape)
        #pdb.set_trace()
        loss_seg, loss_seg2, loss_seg6, loss_seg7 = self.semantic_head.loss(seg_results['seg_pred'], seg_targets, seg_results['seg_pred2'], seg_instance_targets)

        seg_results.update(loss_seg=loss_seg, seg_targets=seg_targets, loss_seg2=loss_seg2, loss_seg6 = loss_seg6, loss_seg7 = loss_seg7)
        return seg_results
        
    def _seg_forward(self, x, is_train):
        #pdb.set_trace()
        seg_pred, seg_pred2, seg_features = self.semantic_head(x, is_train)
        seg_results = dict(seg_pred=seg_pred, seg_pred2=seg_pred2, seg_features=seg_features)
        return seg_results
    ...
    def _fusion_forward_train(self, seg_pred, mask_pred, seg_features, sampling_results, gt_masks, img_metas):
        ...
        fusion_targets, fusion_instance_targets = self.fusion_head.get_targets(gt_masks, device, sampling_results, pad_shape)
        #pdb.set_trace()
        loss_fusion, loss_fusion2, loss_fusion6, loss_fusion7 = self.fusion_head.loss(fusion_results['final_pred'], fusion_targets, fusion_results['final_pred2'], fusion_instance_targets)
        
        fusion_results.update(loss_fusion=loss_fusion, fusion_targets = fusion_targets, loss_fusion2=loss_fusion2, loss_fusion6 = loss_fusion6, loss_fusion7 = loss_fusion7)
    ...
    def _fusion_forward(self, seg_pred, mask_pred, seg_features, pad_shape, boxes, is_Train):
        ...
        final_pred, final_pred2 = self.fusion_head(seg_pred, seg_features, mask_pred, is_Train, boxes)
        fusion_results = dict(final_pred = final_pred, final_pred2 = final_pred2)
        #fusion_results = dict()
        return fusion_results
