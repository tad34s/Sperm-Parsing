...
from mmdet.core.visualization import (
    imshow_det_bboxes,
    imshow_det_bboxes_part,
    imshow_det_bboxes_part100x,
)
import pdb
...
class BaseDetector(BaseModule, metaclass=ABCMeta):
    ...
    def forward_test(self, imgs, img_metas, **kwargs):
        ...
        if num_augs == 1:
            ...
            assert imgs[0].size(0) == 1, (
                f"aug test does not support inference with batch size {imgs[0].size(0)}"
            )
    ...
    def show_result(
        ...
        if out_file is not None:
            show = False
        # draw bounding boxes
        print("segments:", segms.shape, "labels", labels.shape, sep="\n")
        ...
        for i in range(0, 3):
            if segm_result[i] is not None and len(labels) > 0:  # non empty
                if isinstance(segm_result[i][0], torch.Tensor):
                    segm_result[i] = (
                        torch.stack(segm_result[i], dim=0).detach().cpu().numpy()
                    )
                else:
                    segm_result[i] = np.stack(segm_result[i], axis=0)
        # if out_file specified, do not show image in window
        if out_file is not None:
            show = False
        # draw bounding boxes
        img = imshow_det_bboxes_part(
            img,
            bboxes,
            labels,
            segm_result,
        ...
        for i in range(0, 5):
            ...
            if segm_result[i] is not None and len(labels) > 0:  # non empty
                if isinstance(segm_result[i][0], torch.Tensor):
                    ...
                    segm_result[i] = np.stack(segm_result[i], axis=0)
        # segm_result[0] = np.stack(segm_result[0], axis=0)
        # if out_file specified, do not show image in window
        if out_file is not None:
            show = False
        # draw bounding boxes
        img = imshow_det_bboxes_part100x(
            img,
            bboxes,
            labels,
            segm_result,
