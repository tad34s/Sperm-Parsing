...
class PartSegmentationHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(PartSegmentationHead, self).__init__(num_stuff_classes + 1, init_cfg,
                                              loss_seg)
    ...
    def get_targets(self, gt_masks, rcnn_train_cfg):
        gt_seg = []
        for gt_mask in gt_masks:
            mask_target = gt_mask.to_ndarray()
            seg = mask_target[0]
            for i in mask_target.shape[0]:
                  seg = seg | mask_target[i]
            gt_seg.append(seg)
        return gt_seg
